...
document.querySelector(
  "h3"
).innerText = `${currentPlayer.toUpperCase()}'s turn`;
...
function changeCharacter(character) {
  ...
  document.querySelector(
    "h3"
  ).innerText = `${newCharacter.toUpperCase()}'s turn`;
...
function didSomeoneWin(character) {
  ...
  for (let y = 0; y < arr.length; y++) {
    const row = arr[y];
    console.log(
      row[0] == character && row[1] == character && row[2] == character
    );
...
function someoneWon(character) {
  document.querySelector(
    "h2"
  ).innerText = `PLAYER ${character.toUpperCase()} WON!`;
  gameOver();
